...
export default function generate(program) {
  ...
  const generators = {
    ...
    Class(c) {
      output.push(
        `class ${gen(c.id)} {\nconstructor(${
          c.fields.length === 0 ? "" : gen(c.fields[0])
        }) {`
      );
      if (c.fields.length !== 0) {
        c.fields[0].fields.forEach((field) =>
          output.push(`this.${field.id} = ${field.id}`)
        );
        output.push("}");
        c.fields[0].fields.forEach((field) =>
          output.push(`get_${field.id}() { return this.${field.id} }`)
        );
      } else {
        output.push("}");
      }
    ...
    Method(m) {
      output.push(
        `${gen(m.id)}(${m.params.length === 0 ? "" : gen(m.params[0])}) {`
      );
    ...
    Function(d) {
      ...
      output.push(
        `function ${gen(d.id)}(${
          d.params.length === 0 ? "" : gen(d.params[0])
        }) {`
      );
    ...
    For(s) {
      ...
      output.push(
        `for (let ${i} = ${s.initial.id}; ${i} ${op} ${s.final.id}; ${i} = ${i} + ${s.increment.id}) {`
      );
      s.block.forEach((b) => gen(b));
      output.push("}");
    },
    ...
    Assign(a) {
      if (a.target.constructor.name === `ClassAttr`) {
        output.push(
          `${a.target.source.id}.${a.target.method.id} = ${gen(a.source)}`
        );
      } else {
